...
int32_t cold_data_migration(struct ftl_context_t *ptr_ftl_context)
{
    ...
    if(block_valid_copy(ptr_ftl_context, ptr_ftl_context->ec_head_hot_block, ptr_block_buff1))                         //copying valid data of oldest hot block to buffer
    {
        printf("cold mig hot head block copy to buffer failed\n");
    return -1;
    }
    if(block_valid_copy(ptr_ftl_context, ptr_ftl_context->ec_head_cold_block, ptr_block_buff1))                              //copying valid data of youngest cold block to buffer
    {
        printf("cold mig cold head block copy to buffer failed\n");
    return -1;
    }             
    if(page_clean_in_block(ptr_ftl_context, ptr_ftl_context->ec_head_hot_block))
    {                  //erase hot pool oldest block
        printf("cold mig hot head block clean failed\n");
    return -1;
    }
    if(page_clean_in_block(ptr_ftl_context, ptr_ftl_context->ec_head_cold_block))
    {                 //erase cold pool youngest block
        printf("cold mig cold head block clean failed\n");
    return -1;
    }
    if(write_buffer_to_block(ptr_ftl_context, ptr_ftl_context->ec_head_hot_block, ptr_block_buff2))
    {
        printf("cold mig hot head block copy from buffer failed\n");                //write data in buffer to oldest_hot_block
    return -1;
    }
    if(write_buffer_to_block(ptr_ftl_context, ptr_ftl_context->ec_head_cold_block, ptr_block_buff1))
...
bool block_pool_swap(struct ftl_context_t *ptr_ftl_context)
{
    ...
    if (ptr_ftl_context->ec_head_hot_block->nr_erase_cnt > ptr_ftl_context->ec_tail_cold_block->nr_erase_cnt)
        ptr_ftl_context->ec_tail_cold_block = ptr_ftl_context->ec_head_hot_block;
    if (ptr_ftl_context->ec_head_cold_block->nr_erase_cnt < ptr_ftl_context->ec_tail_hot_block->nr_erase_cnt)
        ptr_ftl_context->ec_tail_hot_block = ptr_ftl_context->ec_head_cold_block;
    return 0;
}
...
// uint32_t update_max_min_nr_erase_cnt_in_pool_org(struct ftl_context_t *ptr_ftl_context, int pool, int type, int min_max, int bus, int chip, int block, uint32_t nr_erase_cnt)
// {
//     int i, j, k;
//     struct flash_block_t *ptr_block = NULL;
//     struct flash_ssd_t *ptr_ssd = ptr_ftl_context->ptr_ssd;
