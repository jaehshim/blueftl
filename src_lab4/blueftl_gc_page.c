   ...
   struct flash_block_t *ptr_gc_block)
{
   ...
   {
      printf("0\n");
      gc_write_buff.lpa_arr[gc_counter] = lpa_arr[i];
      memcpy(&(gc_write_buff.buff[FLASH_PAGE_SIZE * gc_counter]), &(buff[FLASH_PAGE_SIZE * i]), FLASH_PAGE_SIZE);
      gc_counter++;
      printf("0.5 %d\n", lpa_arr[i]);
      if (gc_counter == CHUNK_SIZE)
      {
         ...
         printf("3\n");
            ...
            {
               printf("3.0 %d %d\n",gc_write_buff.lpa_arr[k], page_write);
               page_mapping_map_logical_to_physical(ptr_ftl_context, gc_write_buff.lpa_arr[k], ptr_gc_block->no_bus, 
         ...
         printf("3.1\n");
            ...
            for (k = 0; k < ppnum; k++)
            {
               ...
               ptr_gc_block->list_pages[page_write + k].no_logical_page_addr = gc_write_buff.lpa_arr[k];
            }
         printf("3.2\n");
         ...
         else
         {
            ...
            for (k = 0; k < CHUNK_SIZE; k++)
            {
               ...
               blueftl_user_vdevice_page_write(
                  ...
                  FLASH_PAGE_SIZE,
                  (char *)&(gc_write_buff.buff[i*FLASH_PAGE_SIZE]));
...
{ // cache_write_buffer에 담아놨던 정보들 write_buffer로 옮기기
   ...
   {
      memcpy(tmp_ptr, &(gc_write_buff.lpa_arr[i]), sizeof(int32_t));
   ...
   for (i = 0; i < CHUNK_SIZE; i++)
   {
      memcpy(tmp_ptr + FLASH_PAGE_SIZE*i, &(gc_write_buff.buff[FLASH_PAGE_SIZE * i]), FLASH_PAGE_SIZE);
   }
}