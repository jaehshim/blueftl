...
uint32_t init_chunk_table(struct ftl_context_t *ptr_ftl_context)
{
    ...
    ptr_chunk_table->nr_chunk_table_entries =
        ptr_ssd->nr_buses * ptr_ssd->nr_chips_per_bus * ptr_ssd->nr_blocks_per_chip * ptr_ssd->nr_pages_per_block * 2;
    ...
    char *page_data)
{
    ...
    for (i = 0; i < write_counter; i++) // cache
    {
        ...
        if (comp_size != -1)
        {
            ...
            for (i = 0; i < ppnum; i++)
            {
                blueftl_user_vdevice_page_write(
                    ...
                    bus, chip, block, page + i,
                    FLASH_PAGE_SIZE,
                    (char *)compress_buff+(i*FLASH_PAGE_SIZE));
            }
